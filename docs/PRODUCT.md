...
6. **blueprint 문서 자동 업데이트** — 개발 진행 상황(Phase 완료, 결정 사항 등)에 따라 `state.md`·`DIGEST.md` 등 blueprint 산출물을 자동으로 업데이트. 현재 단방향(파일→UI) 원칙의 선택적 확장. ADR 작성 후 진입.
...
| V5 | + blueprint 문서 자동 업데이트 — Phase 완료·결정 사항을 state.md·DIGEST.md에 자동 반영 (ADR 필요) | 별도 결정 |
...
- blueprint 문서 자동 업데이트 → V5 로드맵에 등록. 단방향 원칙 재검토 ADR 작성 후 진입.
